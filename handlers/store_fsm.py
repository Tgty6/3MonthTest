...
async def Admins_load(message: types.Message):
...
def register_handlers_store(dp: Dispatcher):
    dp.register_message_handler(Admins_load, commands=['store'])
    dp.register_message_handler(start_fsm_store, state=['store2'])
